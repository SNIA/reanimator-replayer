...
void ThreadWiseDurationAnalysisModule::considerSyscall(
  ...
  if (module.isTimeable()) {
    ...
    if (threadMap_.find(tid) == threadMap_.end()) {
      ...
      threadMap_[tid] =
          std::map<std::string, ThreadWiseDurationAnalysisStruct>();
    ...
    std::cerr
        << boost::format(
               "<Duration Analysis> Untracked syscall %s is not timeable\n") %
               sys_call_name;
  }
}

void ThreadWiseDurationAnalysisModule::considerTimeElapsed(
...
std::ostream& ThreadWiseDurationAnalysisModule::printMetrics(
...
std::ostream& ThreadWiseDurationAnalysisModule::printGlobalMetrics(
    std::ostream& out) const {
...
std::ostream& ThreadWiseDurationAnalysisModule::printPerSyscallMetrics(
    std::ostream& out) const {
  for (const auto& tm : threadMap_) {
    ...
    for (const auto& am : tm.second) {
      ...
      out << boost::format("%-10s\t%-10u\t%-10u\t%u\n") % am.first %
                 a.min_time_elapsed % a.max_time_elapsed %
