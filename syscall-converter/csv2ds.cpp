...
specs_type parseSpecs(ifstream &input, config_table_type &table) {
  ...
  while (getline(input, line)) {
    // Ignore comments in spec string.
...
public:
  ...
  DataSeriesWriteModule(config_table_type &table, specs_type &specs,
			const std::string xml_dir, const char *outfile) :
    ...
    for (config_table_type::iterator extent = table_.begin();
	 ...
	 extent++) {
      ...
      std::ifstream myExtentXMLFile((xml_dir + extentname + ".xml").c_str());
	...
	extentTypeLibrary.registerTypePtr(myExtentXMLDescription);
      ...
      OutputModuleSpace[extentname] = new OutputModule(outfileSink, *myExtentSeries,
						       myExtentType, target_extent_size);
...
int main(int argc, char *argv[]) {
  ...
  std::string xml_dir(argv[current_arg++]);
  ...
  DataSeriesWriteModule writemod = DataSeriesWriteModule(table, specs, xml_dir,outfile);
