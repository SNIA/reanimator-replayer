...
std::vector<VM_node*>* VM_area::find_left_overlapping_target(void* addr, size_t size) {
  auto addr_int = reinterpret_cast<uint64_t>(addr);
  auto result = new std::vector<VM_node*>();
  std::copy_if(vma.begin(), vma.end(), result->begin(),
               [&](VM_node* vnode) -> bool {
                 auto start_addr =
                     reinterpret_cast<uint64_t>(vnode->traced_start_address);
                 auto end_addr = start_addr + vnode->map_size;
                 return (addr_int + size) > end_addr && addr_int > start_addr && addr_int < end_addr;
               });
  return result;
}
  ...
  std::copy_if(vma.begin(), vma.end(), result->begin(),
               [&](VM_node* vnode) -> bool {
                 auto start_addr =
                     reinterpret_cast<uint64_t>(vnode->traced_start_address);
                 auto end_addr = start_addr + vnode->map_size;
                 return (addr_int + size) > start_addr && (addr_int + size) < end_addr && addr_int < start_addr;
               });
  return result;
}
...
bool VM_area::delete_VM_node(void *addr, size_t size){
  ...
  auto addr_int = reinterpret_cast<uint64_t>(addr);
  std::vector<VM_node*>* left_overlap = find_left_overlapping_target(addr, size);
