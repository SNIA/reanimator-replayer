...
bool process_open_mode(const std::string &mode_arg, 
...
bool process_row(const std::string &in_row, std::string &out_row) {
  ...
  // return info should be located at index 2.
  sys_call_fields.insert(sys_call_fields.begin()+2, std::to_string(ret_val));
...
bool process_open_args(std::string &sys_call_csv_args) {
  ...
  if (sys_call_args.size() == 3) {
    std::string mode_arg = sys_call_args[2];
    // Get modes and store them into mode_vector
    if (process_open_mode(mode_arg, mode_vector) == false) {
      return false;
    }
  }
  ...
  csv_args_stream << path_name << "," << boost::algorithm::join(flags_vector, ",") << ","
 ...
 * from the given system call flags arguments and store those individual
 * open flags into a vector.
 * 
 ...
 *                      position 0 is for O_RDONLY
 *                      position 1 is for O_WRONLY
 *                      ...
