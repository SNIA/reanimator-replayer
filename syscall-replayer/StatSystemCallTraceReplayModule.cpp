...
LStatSystemCallTraceReplayModule(DataSeriesModule &source,
				...
				bool verify_flag,
				int warn_level_flag):
  StatSystemCallTraceReplayModule(source, verbose_flag, verify_flag,
				  warn_level_flag) {
  sys_call_name_ = "lstat";
}

void LStatSystemCallTraceReplayModule::print_specific_fields() {
  StatSystemCallTraceReplayModule::print_specific_fields();
}
...
void LStatSystemCallTraceReplayModule::processRow() {
  ...
  if (verify_ == true) {
	...
	std::cout << "time called:" << std::fixed << time_called()
		  << std::endl;
	std::cout << "Captured lstat content is different from replayed"
		  << "stat content" << std::endl;
	std::cout << "Captured file inode: " << stat_result_ino << ", ";
	std::cout << "Replayed file inode: " << stat_buf.st_ino << std::endl;
        std::cout << "Captured file mode: ";
	...
	std::cout << "Replayed file blocks: " << stat_buf.st_blocks
      ...
      if (verbose_mode()) {
	std::cout << "Verification of lstat buffer succeeded.\n";
      }
    }
  }
}
